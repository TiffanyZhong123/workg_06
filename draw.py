...
def draw_polygons( polygons, screen, color ):
    x = 0
    while(x < len(polygons)-2):
        surface_norm = calculate_normal(polygons,x)
        vector = [0,0,1]
        if(dot_product(vector,surface_norm) >= 0):
            draw_line(int(polygons[x][0]),int(polygons[x][1]),int(polygons[x+1][0]),int(polygons[x+1][1]),screen,color)
            draw_line(int(polygons[x+1][0]),int(polygons[x+1][1]),int(polygons[x+2][0]),int(polygons[x+2][1]),screen,color)
            draw_line(int(polygons[x+2][0]),int(polygons[x+2][1]),int(polygons[x][0]),int(polygons[x][1]),screen,color)
        x+=3
    pass
...
def add_box( polygons, x, y, z, width, height, depth ):
    ...
    add_edge(polygons,x,y,z,x,y-height,z)
    add_edge(polygons,x+width,y,z,x+width,y-height,z)
    add_edge(polygons,x,y,z,x,y,z-depth)

    add_edge(polygons,x,y-height,z,x+width,y-height,z)
    add_edge(polygons,x,y,z-depth,x,y-height,z-depth)
    add_edge(polygons,x,y-height,z-depth,x+width,y-height,z-depth)
    ...
    for x in range(lat_start, lat_stop):
        for y in range(longt_start, longt_stop):
            ...
            if (index % step) + 1 == step:
                add_polygon(polygons,points[index][0],points[index][1],points[index][2],
                            points[x*step][0],points[x*step][1],points[x*step][2],
                            points[(index+step)%(len(points))][0],points[(index+step)%(len(points))][1],points[(index+step)%(len(points))][2])
                add_polygon(polygons,points[x*step][0],points[x*step][1],points[x*step][2],
                            points[(x*step+step)%(len(points))][0],points[(x*step+step)%(len(points))][1],points[(x*step+step)%(len(points))][2],
                            points[(index+step)%(len(points))][0],points[(index+step)%(len(points))][1],points[(index+step)%(len(points))][2])
            else:
                add_polygon(polygons,points[index][0],points[index][1],points[index][2],
                            points[(index+1)%(len(points))][0],points[(index+1)%(len(points))][1],points[(index+1)%(len(points))][2],
